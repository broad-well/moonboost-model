...
from llama_recipes.model_checkpointing import save_model_checkpoint, save_model_and_optimizer_sharded, save_optimizer_checkpoint, save_model_checkpoint_ddp
                    ...
                    if step%train_config.validation_interval==0 and train_config.run_validation:
                        ...
                        if train_config.save_model and eval_epoch_loss < best_val_loss:
                            ...
                            if train_config.use_peft:
                                ...
                                model_to_save.save_pretrained(train_config.output_dir)
                                if train_config.enable_fsdp:
                                    if rank==0:
                                        print(f"PEFT modules are saved in {train_config.output_dir} directory")
                                else:
...
def eval_player_classification_accuracy(model,train_config, dataset_val, local_rank, tokenizer, wandb_run):
    """
    ...
    with MemoryTrace() as memtrace:
        ...
        # for sample in range(len(dataset_val)):
            # batch = dataset_val[sample]
            total_eval_steps += 1
            ...
            for key in batch.keys():
                if train_config.enable_fsdp:
                    batch[key] = torch.tensor(batch[key]).unsqueeze(0).to(local_rank)
                else:
                    if is_xpu_available():
                        batch[key] = torch.tensor(batch[key]).unsqueeze(0).to('xpu:0')
                    else:
                        batch[key] = torch.tensor(batch[key]).unsqueeze(0).to('cuda:0')
            # Ensure no gradients are computed for this scope to save memory
            with torch.no_grad():
                # Forward pass and compute loss
                outputs = model(**batch)
                loss, logits = outputs.loss, outputs.logits #logits should have shape 1, num_classes
...
def eval_player_classification(model,train_config, eval_dataloader, local_rank, tokenizer, wandb_run):
    """
    Evaluates the model on the given dataloader
